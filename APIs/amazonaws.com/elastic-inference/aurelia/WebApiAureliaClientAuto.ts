...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		DescribeAcceleratorOfferings(requestBody: DescribeAcceleratorOfferingsBody, headersHandler?: () => {[header: string]: string}): Promise<DescribeAcceleratorOfferingsResponse> {
			return this.http.post('/describe-accelerator-offerings', JSON.stringify(requestBody), { headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => d.json());
		}
		...
		DescribeAcceleratorTypes(headersHandler?: () => {[header: string]: string}): Promise<DescribeAcceleratorTypesResponse> {
			return this.http.get('/describe-accelerator-types', { headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		...
		DescribeAccelerators(maxResults: string, nextToken: string, requestBody: DescribeAcceleratorsBody, headersHandler?: () => {[header: string]: string}): Promise<DescribeAcceleratorsResponse> {
			return this.http.post('/describe-accelerators?maxResults=' + (maxResults == null ? '' : encodeURIComponent(maxResults)) + '&nextToken=' + (nextToken == null ? '' : encodeURIComponent(nextToken)), JSON.stringify(requestBody), { headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => d.json());
		}
