...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		GenerateDataSet(requestBody: GenerateDataSetRequest, headersHandler?: () => {[header: string]: string}): Promise<GenerateDataSetResult> {
			return Axios.post<GenerateDataSetResult>(this.baseUri + '/#X-Amz-Target=MarketplaceCommerceAnalytics20150701.GenerateDataSet', JSON.stringify(requestBody), { headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => d.data);
		}
		...
		StartSupportDataExport(requestBody: StartSupportDataExportRequest, headersHandler?: () => {[header: string]: string}): Promise<StartSupportDataExportResult> {
			return Axios.post<StartSupportDataExportResult>(this.baseUri + '/#X-Amz-Target=MarketplaceCommerceAnalytics20150701.StartSupportDataExport', JSON.stringify(requestBody), { headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => d.data);
		}
	}
