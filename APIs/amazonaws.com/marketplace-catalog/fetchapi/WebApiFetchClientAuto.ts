export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		DescribeChangeSet(catalog: string, changeSetId: string, headersHandler?: () => {[header: string]: string}): Promise<DescribeChangeSetResponse> {
			return fetch(this.baseUri + '/DescribeChangeSet#catalog&changeSetId?catalog=' + (catalog == null ? '' : encodeURIComponent(catalog)) + '&changeSetId=' + (changeSetId == null ? '' : encodeURIComponent(changeSetId)), { method: 'get', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		...
		DescribeEntity(catalog: string, entityId: string, headersHandler?: () => {[header: string]: string}): Promise<DescribeEntityResponse> {
			return fetch(this.baseUri + '/DescribeEntity#catalog&entityId?catalog=' + (catalog == null ? '' : encodeURIComponent(catalog)) + '&entityId=' + (entityId == null ? '' : encodeURIComponent(entityId)), { method: 'get', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		...
		ListEntities(MaxResults: string, NextToken: string, requestBody: ListEntitiesBody, headersHandler?: () => {[header: string]: string}): Promise<ListEntitiesResponse> {
			return fetch(this.baseUri + '/ListEntities?MaxResults=' + (MaxResults == null ? '' : encodeURIComponent(MaxResults)) + '&NextToken=' + (NextToken == null ? '' : encodeURIComponent(NextToken)), { method: 'post', headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => d.json());
		}
		...
		StartChangeSet(requestBody: StartChangeSetBody, headersHandler?: () => {[header: string]: string}): Promise<StartChangeSetResponse> {
			return fetch(this.baseUri + '/StartChangeSet', { method: 'post', headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => d.json());
		}
	}
