...
namespace My_Demo_Client {
	...
	export class DemoClient {
		 ...
		 */
		GetSnapshotBlock(snapshotId: string, blockIndex: number, blockToken: string, callback: (data : GetSnapshotBlockResponse) => any, headersHandler?: () => {[header: string]: string}) {
			this.httpClient.get(this.baseUri + '/snapshots/' + (snapshotId == null ? '' : encodeURIComponent(snapshotId)) + '/blocks/' + blockIndex + '#blockToken&blockToken=' + (blockToken == null ? '' : encodeURIComponent(blockToken)), callback, this.error, this.statusCode, headersHandler);
		}
		 ...
		 */
		ListChangedBlocks(firstSnapshotId: string, secondSnapshotId: string, pageToken: string, maxResults: number, startingBlockIndex: number, MaxResults: string, NextToken: string, callback: (data : ListChangedBlocksResponse) => any, headersHandler?: () => {[header: string]: string}) {
			this.httpClient.get(this.baseUri + '/snapshots/' + (secondSnapshotId == null ? '' : encodeURIComponent(secondSnapshotId)) + '/changedblocks?firstSnapshotId=' + (firstSnapshotId == null ? '' : encodeURIComponent(firstSnapshotId)) + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)) + '&maxResults=' + maxResults + '&startingBlockIndex=' + startingBlockIndex + '&MaxResults=' + (MaxResults == null ? '' : encodeURIComponent(MaxResults)) + '&NextToken=' + (NextToken == null ? '' : encodeURIComponent(NextToken)), callback, this.error, this.statusCode, headersHandler);
		}
		 ...
		 */
		ListSnapshotBlocks(snapshotId: string, pageToken: string, maxResults: number, startingBlockIndex: number, MaxResults: string, NextToken: string, callback: (data : ListSnapshotBlocksResponse) => any, headersHandler?: () => {[header: string]: string}) {
