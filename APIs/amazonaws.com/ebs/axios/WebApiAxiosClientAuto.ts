...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		GetSnapshotBlock(snapshotId: string, blockIndex: number, blockToken: string, headersHandler?: () => {[header: string]: string}): Promise<GetSnapshotBlockResponse> {
			return Axios.get<GetSnapshotBlockResponse>(this.baseUri + '/snapshots/' + (snapshotId == null ? '' : encodeURIComponent(snapshotId)) + '/blocks/' + blockIndex + '#blockToken&blockToken=' + (blockToken == null ? '' : encodeURIComponent(blockToken)), { headers: headersHandler ? headersHandler() : undefined }).then(d => d.data);
		}
