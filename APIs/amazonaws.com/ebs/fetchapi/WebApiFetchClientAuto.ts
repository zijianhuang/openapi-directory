export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		GetSnapshotBlock(snapshotId: string, blockIndex: number, blockToken: string, headersHandler?: () => {[header: string]: string}): Promise<GetSnapshotBlockResponse> {
			return fetch(this.baseUri + '/snapshots/' + (snapshotId == null ? '' : encodeURIComponent(snapshotId)) + '/blocks/' + blockIndex + '#blockToken&blockToken=' + (blockToken == null ? '' : encodeURIComponent(blockToken)), { method: 'get', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		...
		ListChangedBlocks(firstSnapshotId: string, secondSnapshotId: string, pageToken: string, maxResults: number, startingBlockIndex: number, MaxResults: string, NextToken: string, headersHandler?: () => {[header: string]: string}): Promise<ListChangedBlocksResponse> {
			return fetch(this.baseUri + '/snapshots/' + (secondSnapshotId == null ? '' : encodeURIComponent(secondSnapshotId)) + '/changedblocks?firstSnapshotId=' + (firstSnapshotId == null ? '' : encodeURIComponent(firstSnapshotId)) + '&pageToken=' + (pageToken == null ? '' : encodeURIComponent(pageToken)) + '&maxResults=' + maxResults + '&startingBlockIndex=' + startingBlockIndex + '&MaxResults=' + (MaxResults == null ? '' : encodeURIComponent(MaxResults)) + '&NextToken=' + (NextToken == null ? '' : encodeURIComponent(NextToken)), { method: 'get', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
