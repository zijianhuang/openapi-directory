...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		 ...
		 */
		Search(cursor: string, expr: string, facet: string, fq: string, highlight: string, partial: boolean, q: string, q.options: string, q.parser: QueryParser, return: string, size: number, sort: string, start: number, stats: string, format: string, pretty: string, headersHandler?: () => {[header: string]: string}): Promise<SearchResponse> {
			return this.http.get('/2013-01-01/search#format=sdk&pretty=true&q?cursor=' + (cursor == null ? '' : encodeURIComponent(cursor)) + '&expr=' + (expr == null ? '' : encodeURIComponent(expr)) + '&facet=' + (facet == null ? '' : encodeURIComponent(facet)) + '&fq=' + (fq == null ? '' : encodeURIComponent(fq)) + '&highlight=' + (highlight == null ? '' : encodeURIComponent(highlight)) + '&partial=' + partial + '&q=' + (q == null ? '' : encodeURIComponent(q)) + '&q.options=' + (q.options == null ? '' : encodeURIComponent(q.options)) + '&q.parser=' + q.parser + '&return=' + (return == null ? '' : encodeURIComponent(return)) + '&size=' + size + '&sort=' + (sort == null ? '' : encodeURIComponent(sort)) + '&start=' + start + '&stats=' + (stats == null ? '' : encodeURIComponent(stats)) + '&format=' + (format == null ? '' : encodeURIComponent(format)) + '&pretty=' + (pretty == null ? '' : encodeURIComponent(pretty)), { headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		 ...
		 */
		Suggest(q: string, suggester: string, size: number, format: string, pretty: string, headersHandler?: () => {[header: string]: string}): Promise<SuggestResponse> {
			return this.http.get('/2013-01-01/suggest#format=sdk&pretty=true&q&suggester?q=' + (q == null ? '' : encodeURIComponent(q)) + '&suggester=' + (suggester == null ? '' : encodeURIComponent(suggester)) + '&size=' + size + '&format=' + (format == null ? '' : encodeURIComponent(format)) + '&pretty=' + (pretty == null ? '' : encodeURIComponent(pretty)), { headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		 ...
		 */
		UploadDocuments(format: string, requestBody: UploadDocumentsBody, headersHandler?: () => {[header: string]: string}): Promise<UploadDocumentsResponse> {
			return this.http.post('/2013-01-01/documents/batch#format=sdk&Content-Type?format=' + (format == null ? '' : encodeURIComponent(format)), JSON.stringify(requestBody), { headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => d.json());
		}
	}
