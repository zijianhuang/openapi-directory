...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		 ...
		 */
		Search(cursor: string, expr: string, facet: string, fq: string, highlight: string, partial: boolean, q: string, q.options: string, q.parser: QueryParser, return: string, size: number, sort: string, start: number, stats: string, format: string, pretty: string, headersHandler?: () => HttpHeaders): Observable<SearchResponse> {
		 ...
		 */
		UploadDocuments(format: string, requestBody: UploadDocumentsBody, headersHandler?: () => HttpHeaders): Observable<UploadDocumentsResponse> {
			return this.http.post<UploadDocumentsResponse>(this.baseUri + '/2013-01-01/documents/batch#format=sdk&Content-Type?format=' + (format == null ? '' : encodeURIComponent(format)), JSON.stringify(requestBody), { headers: headersHandler ? headersHandler().append('Content-Type', 'application/json;charset=UTF-8') : new HttpHeaders({ 'Content-Type': 'application/json;charset=UTF-8' }) });
		}
	}
