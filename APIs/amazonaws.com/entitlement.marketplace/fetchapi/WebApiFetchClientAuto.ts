export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		GetEntitlements(requestBody: GetEntitlementsRequest, headersHandler?: () => {[header: string]: string}): Promise<GetEntitlementsResult> {
			return fetch(this.baseUri + '/#X-Amz-Target=AWSMPEntitlementService.GetEntitlements', { method: 'post', headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(requestBody) }).then(d => d.json());
		}
	}
