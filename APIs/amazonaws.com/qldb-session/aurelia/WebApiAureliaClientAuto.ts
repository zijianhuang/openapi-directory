...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		SendCommand(requestBody: SendCommandRequest, headersHandler?: () => {[header: string]: string}): Promise<SendCommandResult> {
			return this.http.post('#X-Amz-Target=QLDBSession.SendCommand', JSON.stringify(requestBody), { headers: headersHandler ? Object.assign(headersHandler(), { 'Content-Type': 'application/json;charset=UTF-8' }): { 'Content-Type': 'application/json;charset=UTF-8' } }).then(d => d.json());
		}
	}
