...
export namespace My_Demo_Client {
	...
	export class DemoClient {
		 ...
		 */
		CreateLifecyclePolicy(requestBody: CreateLifecyclePolicyBody, headersHandler?: () => HttpHeaders): Observable<CreateLifecyclePolicyResponse> {
			return this.http.post<CreateLifecyclePolicyResponse>(this.baseUri + '/policies', JSON.stringify(requestBody), { headers: headersHandler ? headersHandler().append('Content-Type', 'application/json;charset=UTF-8') : new HttpHeaders({ 'Content-Type': 'application/json;charset=UTF-8' }) });
		}
		 ...
		 */
		GetLifecyclePolicies(policyIds: Array<PolicyId>, state: GettablePolicyStateValues, resourceTypes: Array<ResourceTypeValues>, targetTags: Array<TagFilter>, tagsToAdd: Array<TagFilter>, headersHandler?: () => HttpHeaders): Observable<GetLifecyclePoliciesResponse> {
			return this.http.get<GetLifecyclePoliciesResponse>(this.baseUri + '/policies?' + policyIds.map(z => `policyIds=${z}`).join('&') + '&state=' + state + '&' + resourceTypes.map(z => `resourceTypes=${z}`).join('&') + '&' + targetTags.map(z => `targetTags=${z}`).join('&') + '&' + tagsToAdd.map(z => `tagsToAdd=${z}`).join('&'), { headers: headersHandler ? headersHandler() : undefined });
		}
		 ...
		 */
		DeleteLifecyclePolicy(policyId: string, headersHandler?: () => HttpHeaders): Observable<DeleteLifecyclePolicyResponse> {
			return this.http.delete<DeleteLifecyclePolicyResponse>(this.baseUri + '/policies/' + (policyId == null ? '' : encodeURIComponent(policyId)) + '/', { headers: headersHandler ? headersHandler() : undefined });
		}
		 ...
		 */
		GetLifecyclePolicy(policyId: string, headersHandler?: () => HttpHeaders): Observable<GetLifecyclePolicyResponse> {
			return this.http.get<GetLifecyclePolicyResponse>(this.baseUri + '/policies/' + (policyId == null ? '' : encodeURIComponent(policyId)) + '/', { headers: headersHandler ? headersHandler() : undefined });
		}
