export namespace My_Demo_Client {
	...
	export class DemoClient {
		...
		GetLifecyclePolicies(policyIds: Array<PolicyId>, state: GettablePolicyStateValues, resourceTypes: Array<ResourceTypeValues>, targetTags: Array<TagFilter>, tagsToAdd: Array<TagFilter>, headersHandler?: () => {[header: string]: string}): Promise<GetLifecyclePoliciesResponse> {
			return fetch(this.baseUri + '/policies?' + policyIds.map(z => `policyIds=${z}`).join('&') + '&state=' + state + '&' + resourceTypes.map(z => `resourceTypes=${z}`).join('&') + '&' + targetTags.map(z => `targetTags=${z}`).join('&') + '&' + tagsToAdd.map(z => `tagsToAdd=${z}`).join('&'), { method: 'get', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		...
		DeleteLifecyclePolicy(policyId: string, headersHandler?: () => {[header: string]: string}): Promise<DeleteLifecyclePolicyResponse> {
			return fetch(this.baseUri + '/policies/' + (policyId == null ? '' : encodeURIComponent(policyId)) + '/', { method: 'delete', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
		...
		GetLifecyclePolicy(policyId: string, headersHandler?: () => {[header: string]: string}): Promise<GetLifecyclePolicyResponse> {
			return fetch(this.baseUri + '/policies/' + (policyId == null ? '' : encodeURIComponent(policyId)) + '/', { method: 'get', headers: headersHandler ? headersHandler() : undefined }).then(d => d.json());
		}
