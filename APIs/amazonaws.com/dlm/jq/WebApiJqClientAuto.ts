...
namespace My_Demo_Client {
	...
	export class DemoClient {
		 ...
		 */
		CreateLifecyclePolicy(callback: (data : CreateLifecyclePolicyResponse) => any, requestBody: CreateLifecyclePolicyBody, headersHandler?: () => {[header: string]: string}) {
		 ...
		 */
		GetLifecyclePolicies(policyIds: Array<PolicyId>, state: GettablePolicyStateValues, resourceTypes: Array<ResourceTypeValues>, targetTags: Array<TagFilter>, tagsToAdd: Array<TagFilter>, callback: (data : GetLifecyclePoliciesResponse) => any, headersHandler?: () => {[header: string]: string}) {
			this.httpClient.get(this.baseUri + '/policies?' + policyIds.map(z => `policyIds=${z}`).join('&') + '&state=' + state + '&' + resourceTypes.map(z => `resourceTypes=${z}`).join('&') + '&' + targetTags.map(z => `targetTags=${z}`).join('&') + '&' + tagsToAdd.map(z => `tagsToAdd=${z}`).join('&'), callback, this.error, this.statusCode, headersHandler);
		}
		 ...
		 */
		DeleteLifecyclePolicy(policyId: string, callback: (data : DeleteLifecyclePolicyResponse) => any, headersHandler?: () => {[header: string]: string}) {
		 ...
		 */
		GetLifecyclePolicy(policyId: string, callback: (data : GetLifecyclePolicyResponse) => any, headersHandler?: () => {[header: string]: string}) {
			this.httpClient.get(this.baseUri + '/policies/' + (policyId == null ? '' : encodeURIComponent(policyId)) + '/', callback, this.error, this.statusCode, headersHandler);
		}
